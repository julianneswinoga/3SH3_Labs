...
int main(void) {
	...
	for (line = 0;
		 fgets(buf, sizeof(buf) / sizeof(char), (FILE *)inputFile) != NULL;
		 line++) {
	...
	pthread_mutex_init(&mutex_phaseComplete,
					   NULL); // Init mutex for phase complete
	pthread_cond_init(&condition_phaseComplete,
					  NULL); // Init condition for phase complete
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);

	for (int i = 0; i < meshSize; i++) { // Create $(meshSize) threads
		threadIds[i] = i;
		pthread_create(&threads[i], &attr, meshThreadFunc,
					   (void *)(uintptr_t)threadIds[i]);
	}
